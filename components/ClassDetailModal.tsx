...
import { schoolAddress, formatDatePl, formatDuration, splitInstructors, nextOccurrences } from "@/lib/schedule";
...
export function ClassDetailModal({ row, allRows, onClose }: { row: ClassRow; allRows: ClassRow[]; onClose: () => void }) {
  ...
  const pricing = SCHOOL_PRICING[row.school];
    ...
    >
      ...
      >
        ...
        <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 rounded-lg border border-line bg-black/20 p-3.5 sm:grid-cols-3">
          ...
          {pricing && (
            <InfoRow label="Cena">
              od {pricing.fromPrice}{" "}
              <Link href="/cenniki" className="text-accent hover:text-accent-peach">
                cennik →
              </Link>
            </InfoRow>
          )}
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <PlusButton label="Dodaj do mojego planu" active={plannedClassIds.has(favoriteId)} onToggle={() => togglePlanClass(favoriteId)} />
                ...
                return (
                  <div key={name} className="flex gap-3 rounded-lg border border-line bg-black/20 p-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-violet">{row.school}</p>
                      {bio && <p className="mt-1 line-clamp-3 whitespace-pre-line text-xs text-zinc-300">{bio}</p>}
                      {styles.length > 0 && (
        ...
        <div className="mt-4 rounded-lg border border-line bg-black/20 p-3">
          <p className="font-heading text-sm font-semibold text-zinc-50">{row.school}</p>
        ...
        >
          Zobacz ten grafik na stronie szkoły ↗
        </a>
          ...
          <div className="mt-5 border-t border-line pt-4">
            ...
            <div className="mt-2 flex flex-col divide-y divide-line">
              {moreFromInstructor.map(({ row: r, label }) => (
                <div key={`${r.school}-${r.id}`} className="flex items-center gap-3 py-2 first:pt-0 last:pb-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-[11px] text-muted">{r.school}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {otherInSchool.length > 0 && (
          <div className="mt-4 border-t border-line pt-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted">Inne zajęcia w {row.school}</p>
            <div className="mt-2 flex flex-col divide-y divide-line">
