...
export function AddToCalendarButton({ row, variant = "pill" }: { row: ClassRow; variant?: "pill" | "tile" }) {
    ...
    <div ref={ref} className="relative">
      {variant === "tile" ? (
        <button
          ...
          className="flex w-full flex-col items-center gap-1 rounded-xl border border-line bg-black/30 px-2 py-2.5 text-xs font-semibold text-zinc-200 transition-colors hover:border-violet/50 hover:text-violet"
        >
      ...
      {open && (
        <div
          ...
          className={`absolute z-30 mt-1.5 w-56 overflow-hidden rounded-xl border border-line bg-zinc-900 shadow-[0_8px_24px_rgba(0,0,0,0.5)] ${variant === "tile" ? "left-0" : "right-0"}`}
        >
          <a
