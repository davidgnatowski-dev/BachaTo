...
export default function ImprezyPage() {
  const events = getUpcomingEvents().filter((e) => e.category === "social");
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-6 px-4 py-8 sm:px-6">
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="font-heading text-2xl font-semibold tracking-tight text-zinc-50">Imprezy bachatowe</h1>
          <p className="mt-1 text-sm text-muted">Socjale i imprezy w całej Polsce, w tym cykliczne spotkania społeczności.</p>
        </div>
        <RefreshButton />
      </header>

      <TabNav active="imprezy" />
      ...
      {events.length === 0 ? (
        <p className="rounded-xl border border-dashed border-line p-8 text-center text-sm text-muted">
          Brak imprez do wyświetlenia. Uruchom scraping, żeby je zobaczyć.
        </p>
      ) : (
        <EventsExplorer rows={events} lockedCategory="social" />
      )}
    </div>
  );
}
